...
type AdminTab = 'results' | 'points' | 'calendar' | 'drivers';
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 max-w-7xl">
            ...
            <div className="flex border-b border-[var(--border-color)] mb-6 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('points')} className={`px-4 py-2 text-lg font-medium transition-colors whitespace-nowrap ${activeTab === 'points' ? 'f1-red-text border-b-2 border-[var(--accent-red)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}>Puntos</button>
...
const PointsManagement: React.FC = () => {
    ...
    const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if(query.length > 0) {
            setSuggestions(users.filter(u => u.role === 'user' && u.name.toLowerCase().includes(query.toLowerCase())));
        } else {
            setSuggestions([]);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedUser || points === 0 || !reason.trim() || !adminUser) {
            ...
            <form onSubmit={handleSubmit} className="mb-8 bg-[var(--background-light)]/50 p-4 rounded-lg space-y-4 border border-[var(--border-color)]">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                     ...
                     <div>
                        ...
                        <input type="number" id="points" value={points} onChange={(e) => setPoints(parseInt(e.target.value, 10) || 0)} className="w-full p-2 bg-[var(--background-light)] rounded border border-[var(--border-color)] focus:ring-2 focus:ring-[var(--accent-red)]" />
                     </div>
                     <div>
