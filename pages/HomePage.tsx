...
const HomePage: React.FC = () => {
    ...
    const [leaderboard, setLeaderboard] = useState<SeasonTotal[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (lastFinishedGp) {
                    setLastGp(lastFinishedGp);
                    setLastResult(officialResults.find(r => r.gpId === lastFinishedGp.id) || null);
                }
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8 max-w-7xl space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Left Column: Last GP Results & Next GP */}
                <div className="lg:col-span-1 space-y-8">
                    ...
                    {lastGp && lastResult && (
                        <div className="bg-[var(--background-medium)] p-6 rounded-xl border border-[var(--border-color)]">
                            <h2 className="text-2xl font-bold mb-4 f1-red-text">Resultados: {lastGp.name}</h2>
                            <div className="space-y-4">
                               <h3 className="font-semibold text-lg text-[var(--text-primary)]">Carrera</h3>
                               <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                   {lastResult.racePodium?.map((driverId, i) => <PodiumCard key={i} position={i+1} driverId={driverId} drivers={drivers} teams={teams}/>)}
                               </div>
                               <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                   <ResultCard title="Pole Position" driverId={lastResult.pole} drivers={drivers} teams={teams}/>
                                   <ResultCard title="Vuelta Rápida" driverId={lastResult.fastestLap} drivers={drivers} teams={teams}/>
                                   <ResultCard title="Piloto del Día" driverId={lastResult.driverOfTheDay} drivers={drivers} teams={teams}/>
                               </div>
                               {lastGp.hasSprint && lastResult.sprintPodium && (
                                   <>
                                    <h3 className="font-semibold text-lg text-[var(--text-primary)] pt-4">Sprint</h3>
                                     <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                        {lastResult.sprintPodium?.map((driverId, i) => <PodiumCard key={`sprint-${i}`} position={i+1} driverId={driverId} drivers={drivers} teams={teams}/>)}
                                    </div>
                                   </>
                               )}
                            </div>
                    ...
                    {nextGp && (
                        <div className="bg-gradient-to-br from-[var(--accent-red)] to-red-800 p-6 rounded-xl text-white text-center shadow-2xl shadow-red-900/20">
                           ...
                           <p className="text-3xl font-light mt-2 mb-4">{nextGp.name}</p>
                           <Countdown targetDate={nextGp.events.race} />
                           <Link to={`/predict/${nextGp.id}`} className="mt-6 inline-block bg-white text-[var(--accent-red)] font-bold py-3 px-8 rounded-full hover:bg-gray-200 transition-transform hover:scale-105 shadow-lg">
                               Hacer Predicción
                           </Link>
                        </div>
                    )}
                ...
                {/* Right Column: General Leaderboard */}
                                ...
                                {loading && leaderboard.length === 0 && (
                                    Array.from({length: 5}).map((_, i) => (
                                         <tr key={i} className="border-b border-[var(--border-color)]">
                                            ...
                                            <td className="p-2"><div className="w-10 h-10 bg-[var(--background-light)] rounded-full"></div></td>
                                            <td className="p-2"><div className="h-5 w-24 bg-[var(--background-light)] rounded"></div></td>
                                            <td className="p-3 text-right"><div className="h-6 w-8 bg-[var(--background-light)] rounded ml-auto"></div></td>
                                            <td className="hidden md:table-cell p-3 text-center"><div className="h-5 w-4 bg-[var(--background-light)] rounded mx-auto"></div></td>
                                            <td className="hidden md:table-cell p-3 text-center"><div className="h-5 w-4 bg-[var(--background-light)] rounded mx-auto"></div></td>
                                        </tr>
                                    ))
                                )}
